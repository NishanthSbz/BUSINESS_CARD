...
function handleResponsiveLayout() {
  ...
  function adjustLayout() {
    ...
    if (screenWidth <= 768) {
      ...
      frontContent.style.flexDirection = 'row';
      if (detailsSection) {
    ...
    const aspectRatio = 500 / 280; // Original aspect ratio
...
// Auto-flip demonstration (optional)
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const cardContainer = document.querySelector('.card-container');
  cardContainer.style.animation = 'cardEntrance 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
  
  // Preload hover states for smooth interactions
  const profileImage = document.querySelector('.profile-image');
  if (profileImage) {
    profileImage.style.willChange = 'transform';
  }
  
  console.log('🎴 Premium Digital Business Card loaded successfully!');
});
